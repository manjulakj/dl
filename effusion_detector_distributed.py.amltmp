...
def prepare_model(img_rows, img_cols, img_channels, nb_classes):
   ...
   opt = tf.optimizers.SGD(1.0 * hvd.size())
   ...
   cnn.compile(optimizer='sgd', loss='categorical_crossentropy', metrics=['accuracy', tf.keras.metrics.AUC()], experimental_run_tf_function=False)
   print(cnn.summary())
   return cnn;
...
n_inputs = 256 * 256
n_outputs = 2
n_epochs = int(math.ceil(12.0 / hvd.size())) # Horovod: adjust number of epochs based on number of GPUs.
...
callbacks_list = [auc_logger, decay, broadcastGV]
...
history = neural_net.fit(training_generator, epochs=10, validation_data=validation_generator, verbose=2, callbacks=callbacks_list)
